...
export default class Vector {
    ...
    static interp(vectorA: Vector, vectorB: Vector, value: number): Vector {
        return {
    ...
    static interpKeyframes(keyframes: Keyframe[], value: number): Vector {
        if (keyframes.length == 0) {
            throw new Error('keyframes cannot be empty')
        }
        keyframes.sort((a,b) => (a.p - b.p))

        let result: Vector = keyframes[0]
     ...
     * and ending at line2.
     */
    static shortestVectorToLine(point: Vector, line1: Vector, line2: Vector): Vector {
...
interface Keyframe extends Vector {
    p: number
}
