export async function delay(durationMillis: number): Promise<void> {
...
export class CancellableTask {
    ...
    constructor(runFunc: ((task: CancellableTask) => Promise<void>)) {
        ...
        this.completionPromise.catch((error: Error) => {
...
export function map(x: number, options: {from?: [number, number],
                                         to?: [number, number],
